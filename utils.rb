    ...
    class GameSDK
        ...
        def getProductGroup(proj)
            if proj.products_group==nil
                product_group = Utils::GameSDK.creatNewGroup(proj,"xg_products")
                else
        ...
        def copyBuildPhase(newTarget,proj,basicTarget)
            ...
            if file.settings!=nil && file.settings['ATTRIBUTES']!=nil
                newBuildFile=proj.new("PBXBuildFile")
                newBuildFile.file_ref=file.file_ref
                newBuildFile.settings={}
                newBuildFile.settings['ATTRIBUTES'] = file.settings['ATTRIBUTES']
            else
                newBuildFile = file
            end
                newTarget.frameworks_build_phase.files << newBuildFile
            end
        ...
        def addOtherLinkFlag(newTarget,flagList)
            ...
            Utils::GameSDK.addHashwithListValue(flagList1,"OTHER_LDFLAGS","$(inherited)")
            Utils::GameSDK.addHashwithListValue(flagList2,"OTHER_LDFLAGS","$(inherited)")
            for flag in flagList
                Utils::GameSDK.addHashwithListValue(flagList1,"OTHER_LDFLAGS",flag)
                Utils::GameSDK.addHashwithListValue(flagList2,"OTHER_LDFLAGS",flag)
            end
        end
        
        def addGameKitKey(hashPlist)
            if hashPlist["UIRequiredDeviceCapabilities"]!=nil
                hashPlist["UIRequiredDeviceCapabilities"]<<"gamekit"
            end
        end
        ...
        def  addLibSearchPath(newTarget,pathLit)
            flagList1 = newTarget.build_settings("Debug")
            flagList2 = newTarget.build_settings("Release")
            Utils::GameSDK.addHashwithListValue(flagList1,"LIBRARY_SEARCH_PATHS","$(inherited)")
            Utils::GameSDK.addHashwithListValue(flagList2,"LIBRARY_SEARCH_PATHS","$(inherited)")
            for path in pathLit
                Utils::GameSDK.addHashwithListValue(flagList1,"LIBRARY_SEARCH_PATHS",path)
                Utils::GameSDK.addHashwithListValue(flagList2,"LIBRARY_SEARCH_PATHS",path)
            end
        ...
        def addHeaderSearchPath(newTarget,pathLit)
            flagList1 = newTarget.build_settings("Debug")
            flagList2 = newTarget.build_settings("Release")
            Utils::GameSDK.addHashwithListValue(flagList1,"HEADER_SEARCH_PATHS","$(inherited)")
            Utils::GameSDK.addHashwithListValue(flagList2,"HEADER_SEARCH_PATHS","$(inherited)")
            for path in pathLit
                Utils::GameSDK.addHashwithListValue(flagList1,"HEADER_SEARCH_PATHS",path)
                Utils::GameSDK.addHashwithListValue(flagList2,"HEADER_SEARCH_PATHS",path)
            end
        end
        ...
        def  addFrameSearchPath(newTarget,pathLit)
            flagList1 = newTarget.build_settings("Debug")
            flagList2 = newTarget.build_settings("Release")
             Utils::GameSDK.addHashwithListValue(flagList1,"FRAMEWORK_SEARCH_PATHS","$(inherited)")
            Utils::GameSDK.addHashwithListValue(flagList2,"FRAMEWORK_SEARCH_PATHS","$(inherited)")
            for path in pathLit
                Utils::GameSDK.addHashwithListValue(flagList1,"FRAMEWORK_SEARCH_PATHS",path)
                Utils::GameSDK.addHashwithListValue(flagList2,"FRAMEWORK_SEARCH_PATHS",path)
            end
        ...
        def setBuildPharse(newTarget,sdk_group,path)
            ...
            # flag = Utils::GameSDK.isFileExist(sdk_group.files,fileName)
            ...
            flag = Utils::GameSDK.isFileExist(newTarget.frameworks_build_phase.files,fileName)
            if flag=="no"
                # lib=sdk_group.new_file(path)
        ...
        def setResoucePharse(newTarget,sdk_group,path)
            ...
            flag = Utils::GameSDK.isFileExist(newTarget.resources_build_phase.files,fileName)
...
#            flag = Utils::GameSDK.isFileExist(sdk_group.files,fileName)
            ...
            flag = Utils::GameSDK.isFileExist(newTarget.source_build_phase.files,fileName)
...
#                src = sdk_group.new_file(path)
        ...
        def makeUnitySetting(project_path,proj,newTarget)
            ...
            if is_unity_flag
                ...
                flag2 = Utils::GameSDK.isFileExist(newTarget.resources_build_phase.files,fileName)
                res =nil
                ...
                Utils::GameSDK.deletePathQuote(lib_search_settings_debug);
                Utils::GameSDK.deletePathQuote(lib_search_settings_release);
                p "unity add ok...."
            end
        end
